...
class Player(Character):
  ...
  def explore(self):
    if self.state != 'normal':
      ...
      print "%s explores a very dark path." % self.name
      if randint(0, 1):
        self.enemy = Enemy(self)
...
print "%s enters hell, searching for who know's what." % p.name
  ...
  if len(args) > 0:
    ...
    for c in Commands.keys():
      if args[0] == c[:len(args[0])]:
        ...
        break
    if not commandFound:
      print "%s doesn't understand the suggestion." % p.namequit